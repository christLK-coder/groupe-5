...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM MESSAGE m
                       JOIN CONVERSATION c ON m.id_conversation = c.id_conversation
                       WHERE c.id_medecin = ? AND m.type_expediteur = 'patient'
                       AND m.date_message > COALESCE((SELECT MAX(date_message)
                                                     FROM MESSAGE m2
                                                     WHERE m2.id_conversation = c.id_conversation
                                                     AND m2.type_expediteur = 'medecin'), '2000-01-01')");
$stmt->execute([$id_medecin]);
        ...
        <div class="container-fluid">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-4 col-sm-12">
                    <div class="card">
                        <h3>Note Moyenne</h3>
                        <p id="note-moyenne"><?= $note_moyenne ?> / 5</p>
                    </div>
                </div>
            </div>
