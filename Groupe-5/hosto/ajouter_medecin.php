...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES['image_service']) && $_FILES['image_service']['error'] === 0) {
        $image_service = basename($_FILES['image_service']['name']);
        move_uploaded_file($_FILES['image_service']['tmp_name'], "New folder/" . $image_service);

        // Mise à jour du service sélectionné avec l'image
        $stmtUpdateService = $conn->prepare("UPDATE services SET image_service = ? WHERE id_service = ?");
        $stmtUpdateService->execute([$image_service, $id_service]);
    }
...
<form method="POST" enctype="multipart/form-data">
    ...
    <!-- Sélection d'une image pour le service -->
    <label>Image pour ce service (optionnel) :</label>
    ...
    <label>Nom du service (texte libre) :</label>
    <input type="text" name="service">

    <label>Nom de la spécialité (texte libre) :</label>
