...
// --- MODIFIED SQL QUERY TO INCLUDE AVERAGE RATING ---
$stmt = $pdo->prepare("
    SELECT
        m.*,
        services.nom_service AS nom_service,
        specialite.nom AS nom_specialite,
        AVG(note.note) AS moyenne_note_medecin  -- Add this line to get the average note
    FROM medecin m
    LEFT JOIN services ON m.id_service = services.id_service
    LEFT JOIN specialite ON m.id_specialite = specialite.id_specialite
    LEFT JOIN note ON m.id_medecin = note.id_medecin -- Join with the note table
    WHERE m.id_medecin = :id
    GROUP BY m.id_medecin -- Group by doctor to correctly calculate AVG(note)
");
...
<head>
    ...
    <style>
        ...
        /* --- New style for rating display --- */
        .rating-display {
            font-weight: bold;
            color: #ffa500; /* Orange color for stars/rating */
        }
        /* --- End new style --- */
    </style>
</head>
...
<div class="card">
    ...
    <div class="info">
        ...
        <p><i class="fas fa-star"></i> <strong>Note Moyenne :</strong>
            <span class="rating-display">
                <?php
                if ($medecin['moyenne_note_medecin'] !== null && $medecin['moyenne_note_medecin'] > 0) {
                    echo round($medecin['moyenne_note_medecin'], 1) . ' / 5';
                } else {
