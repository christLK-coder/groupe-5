...
if (isset($_POST['register'])) {
    ...
    $latitude = $_POST['latitude'] ?? null;   // Récupérer la latitude, utiliser null si non présente
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO PATIENT (nom, prenom, email, mot_de_passe, telephone, adresse, image_patient, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$nom, $prenom, $email, $mot_de_passe, $telephone, $adresse, $image_patient, $latitude, $longitude]);
...
if ($user && password_verify($password, $user['mot_de_passe'])) {
    ...
    $_SESSION['latitude'] = $user['latitude'] ?? null;   // ADD THIS (if you implemented geolocation)
    $_SESSION['longitude'] = $user['longitude'] ?? null; // ADD THIS (if you implemented geolocation)
    ...
    <div class="container">
      <div class="forms-container">
        <div class="signin-signup">
          <form method="post" action="login.php" class="sign-in-form" enctype="multipart/form-data">
          ...
          <form method="post" action="login.php" class="sign-up-form" enctype="multipart/form-data">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fonction de succès de la géolocalisation
    ...
    // Fonction d'erreur de la géolocalisation
    function showError(error) {
        switch(error.code) {
            ...
            case error.UNKNOWN_ERROR:
                console.error("Une erreur inconnue est survenue lors de la géolocalisation.");
                break;
        }
        // Gérer le cas où la géolocalisation échoue (par exemple, laisser les champs vides ou demander manuellement)
    }

    // Déclenchez la géolocalisation lorsque le formulaire d'inscription est prêt ou lorsque l'utilisateur clique sur le bouton d'inscription
    // Idéalement, déclenchez-la lorsque l'utilisateur passe au formulaire d'inscription
    ...
    // Ou déclenchez-le dès que la page est chargée si vous voulez le faire au plus tôt
    // getLocation(); // Décommenter si vous voulez essayer de l'obtenir immédiatement au chargement de la page
});
</script>
